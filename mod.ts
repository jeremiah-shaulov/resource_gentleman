/**	How do ladies and gentlemen manage their resource handles?
	...
	To create a reference counted object, use {@link rc()} function from this library, and bind it's result to a "using" (or "await-using") variable.
	...
	The {@link rc()} function takes a disposable or async-disposable resource and returns an identical object, with a reference counter attached to it.
	...
	If the object given to {@link rc()} is already a reference counted object, the counter is incremented and the same object instance is returned.
	...
	Technically, the object returned by {@link rc()} is a `Proxy` wrapper around the original object.
	It adds the reference counter, and 1 additional method to the object:

	- {@link Rc.rcKeep()} - This method can be used to keep the object alive for a longer time.
	If during this time you rebind the object to another "using" variable, it will remain alive.
	{@link Rc.rcKeep()} registers a callback with `setTimeout(callback, 0)`,
	...
	Good news is that you can approach {@link use()} function from this library to take care of business.
...
export {type Rc, rc} from './private/rc.ts';
export {use} from './private/use.ts';
